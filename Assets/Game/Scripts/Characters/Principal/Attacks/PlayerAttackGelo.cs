...
public class PlayerAttackGelo : PlayerAttack
{
    ...
    public override void Attack()
    {
        if (this.enabled)
        {
            _poolManager.GetObject(prefabAtaqueNormal.tagPool, pontoSaidaCima.position, Quaternion.identity);
            _poolManager.GetObject(prefabAtaqueNormal.tagPool, pontoSaidaMeio.position, Quaternion.identity);
            _poolManager.GetObject(prefabAtaqueNormal.tagPool, pontoSaidaBaixo.position, Quaternion.identity);
        }
    }
}